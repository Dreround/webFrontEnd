...
import { loginVerify } from "../api/supermanage";
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          params.append("user_name", this.ruleForm.username);
          params.append("password", this.ruleForm.password);
          this.logining = true;
          // console.log(formName.password);
          loginVerify(params) // this.editForm
            .then((res) => {
              if (res.data.code == 200) {
                this.$store.commit("login", "true");
                this.$router.push({ path: "/goods/oldman" });
